...
<textarea type="text" class="form-control {{ $tiny ? 'tinyeditor' : '' }} @error($name) is-invalid @enderror"  id="{{ $name }}" name="{{ $name }}" placeholder="{{ $placeholder }}" rows="5">{{ old($name, $oldval) }}</textarea>
